...
void loop_body(int64_t start, int64_t num ) {
  ...
  for (int i=start; i<start+num; i++) {
    ...
    Grappa_delegate_fetch_and_add_word( make_global( &count, 1 ), 1 );  // force a suspend 
    sem->release(1);
  }
}

void spawn_private_task(int64_t a, int64_t b) {
  Grappa_privateTask( &async_parallel_for<&loop_body, &spawn_private_task, ASYNC_PAR_FOR_DEFAULT>, a, b );
}
...
void user_main( void * args ) {
  ...
  BOOST_CHECK( size == Grappa_delegate_read_word( make_global( &count, 1 ) ));

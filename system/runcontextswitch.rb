...
params = {
    ...
    num_total_workers: [1<<5, 1<<7, 18000, 65536,100000, 400000],# 1<<5, (1<<5)+(1<<4), 1<<6, 1<<7, 1<<8, (1<<8)+(1<<7), 1<<9, 1<<10, (1<<10)+(1<<9), 1<<11, (1<<11)+(1<<10), 1<<12, (1<<12)+(1<<11), 1<<13, (1<<14)+(1<<13), 1<<15,(1<<15)+(1<<14), 1<<16, (1<<16)+(1<<15), 1<<17, 1<<18],# 48, 56, 64, 128, 256, 512, 600, 650, 700, 750, 800, 850, 900, 950, 1100,1200,1300,1400,1600,1800,2100,2300, 2800, 3100, 3600, 4000],#, 4500, 5000, 5500, 6000, 6500, 15000, 18000, 20000, 24000, 26000, 30000, 40000,60000],
    ...
    machine: ['cluster'],
    problem: ['yields'],
    problem_flavor: ['new-worker-multi-queue-prefetch2x'],

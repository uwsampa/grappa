...
namespace Grappa {
  ...
  void init_shared_pool() {
    ...
    VLOG(2) << "initialized shared_pool @ " << shared_pool << ", buf:" << (void*)shared_pool->buffer;
  }
  ...
  void* SharedMessagePool::allocate(size_t size) {
    ...
    if (this->remaining() < size) {
      ...
      if (unused_pools.size() > 0) {
        ...
        VLOG(1) << "found pool @ " << shared_pool << " allocated:" << shared_pool->allocated << "/" << shared_pool->buffer_size << ", count=" << allocated_count << " @ " << this;
        unused_pools.pop();
      } else {
        ...
        shared_message_pools_allocated++;
        VLOG(2) << "created new shared_pool @ " << shared_pool << ", buf:" << shared_pool->buffer;
      }
      ...
      allocated_count++;
      this->to_send++;
      return PoolAllocator<MessageBase>::allocate(size);

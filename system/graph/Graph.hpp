...
>>>>>>> master
    ...
    SymmetricAddress<Graph<VV,E>> transform(F f) {
      ...
      return static_cast<SymmetricAddress<Graph<VV,E>>>(self);
    }
  ...
  template< typename G >
  struct AdjIterator {
    SymmetricAddress<G> g;
    VertexID i;
    AdjIterator(SymmetricAddress<G> g, VertexID i): g(g), i(i) {}
  };
...
#undef OVERLOAD
  ...
  namespace impl {
    ...
    void forall(SymmetricAddress<G> g, F body,
    ...
    template< GlobalCompletionEvent * C, int64_t Threshold, typename G, typename F >
    void forall(SymmetricAddress<G> g, F body, void (F::*mf)(typename G::Vertex&) const) {
    ...
    template< GlobalCompletionEvent * C, int64_t Threshold, typename G, typename F >
    void forall(SymmetricAddress<G> g, F loop_body,
  ...
  void forall(SymmetricAddress<Graph<V,E>> g, F loop_body) {
  ...
  SymmetricAddress<Graph<V,E>> Graph<V,E>::create(const TupleGraph& tg,

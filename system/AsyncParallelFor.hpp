...
// start is a pointer (to save space)
template < typename T,
           void (*LoopBody)(GlobalAddress<T>,int64_t),  // Must be linear address to be safe
           void (*Spawn)(GlobalAddress<T>,int64_t),
           int64_t Threshold >
void async_parallel_for( GlobalAddress<T> start, int64_t iterations) {
  DVLOG(5) << "[" << start << "," << start+iterations << ")";
  if ( iterations == 0 ) { // TODO: remove this redundant branch
    return;
  } else if ( Threshold == ASYNC_PAR_FOR_DEFAULT ) { // template specialization that should compile out
  ...
  // go down left half
  async_parallel_for<T,LoopBody,Spawn,Threshold>( start, (iterations+1)/2 );
}

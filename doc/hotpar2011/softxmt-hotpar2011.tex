...
gets quickly out of hand. A Yahoo snapshot of the World Wide Web graph
...
\todo{decide whether to change these 2 paragraphs to be inclusive of
  ...
  actually change.}  We treat context switches as function calls, as
...
107 \mrps. We believe this limit is due to the core having only enough {\em line fill
...
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=0.45\textwidth]{figures/cachepressurenewview-edited.pdf}
  \end{center}
  \vspace{0.0in}
...
We varied the size of the per-thread local working set from 2KB
...
we referenced the second chip's memory and also had coroutine scheduler include a delay
before a coroutine can be rescheduled, imitating the network transit
delay even though we are accessing local memory. We assume 1.1 $\mu$s interface and 400 ns switch latencies in each direction
...
\begin{figure}[h]
  ...
  \end{center}
  \vspace{-0.3in}
  \caption{Pointer chasing across QPI link and with simulated network delay. Used six cores and varied the number of coroutines per core.}
  \label{fig:network-delay}
\end{figure}

The results of the experiment are shown in
Figure~\ref{fig:network-delay}. The runtime can still reach a
maximum rate of 173 \mrps\footnote{The throughput keeps increasing up to around 564 total concurrent references offered (at which point QPI is saturated), far more than the 36 we know the chip to support. This is because we have modeled the network latency by forcing coroutines to wait the extra time before using the data, so the physical miss buffers on our machine are not tied up any longer than usual.}, saturating the QPI link. It reaches the estimated network rate of 100 \mrps with about 53 coroutines per core. 
...
\todo{integrate this paragraph with previous ones instead}

Figure~\ref{fig:cache-pressure}b shows the results of the cache pressure experiment for 48 coroutines per core, about the number of contexts needed to reach the network rate in the delay experiment. We observe that when performing up to 8 local updates per remote reference, smaller working sets still allow the desired throughput. When performing 1 to 2 updates, we can have working sets of up to 32KB per coroutine while maintaining throughput.
...
different goal \todo{we didnt say what their goal is}; we want many lightweight contexts to tolerate memory
latency.

...
difficult to lay out with locality: every node needs to be near every
other node. Their power law distribution means that a few vertices
...
nodes.  This memory manager is the only component that might require
hardware, but we are exploring a mix of hardware and software
implementations.
...
\todo{explain why global address space is good}
...
many coroutines can be active without monopolizing the cache. Second,
...
on memory concurrency: section \todo{cite evaluation} finds a limit of

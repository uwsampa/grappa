...
\subsection{Challenges in using commodity \textls*{\small SSD}s}
...
%\paragraph{\textls*{\small SSD}s assume locality.} In our benchmarks, we find the average request size to be
\noindent{\bf \textls*{\small SSD}s assume locality.} In our benchmarks, we find the average request size to be
less than 64 bytes. Today's \textls*{\small SSD}s are optimized for reads in the
kilobyte range.  

%\paragraph{\textls*{\small SSD}s have a low request rate.} The fastest~\cite{fusionio} can
\noindent{\bf \textls*{\small SSD}s have a low request rate.} The fastest~\cite{fusionio} can
...
%\paragraph{\textls*{\small SSD}s have high overhead.} Commodity \textls*{\small SSD}s are usually treated
\noindent{\bf \textls*{\small SSD}s have high overhead.} Commodity \textls*{\small SSD}s are usually treated
...
A natural first step would be to add one or more \textls*{\small SSD}s to each Grappa
...
generate 100 million random 8-byte requests per second, but most \textls*{\small SSD}s
...
100 million per second, the \textls*{\small SSD}s must support 2000 concurrent
requests. Again, existing \textls*{\small SSD}s support orders of magnitude less request concurrency.

We see three ways to address this disparity.

%\paragraph{Scaling up the Flash array}
\noindent{\bf Scaling up the Flash array}
Existing Flash-based \textls*{\small SSD}s are designed for large block requests. An
...
collaboration between our group and the \textls*{\small NVMW} community.

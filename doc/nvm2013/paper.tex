...
place the entire data set on \textls*{\small SSD} or \textls*{\small PCM};
...
data into \textls*{\small RAM}; save energy per available byte;
...
this shared heap in Grappa programs are done through an \textls*{\small API}, so this change could be made transparent to the programmer. If the
...
\subsection{Challenges in using commodity \textls*{\small SSD}\hspace{0.5pt}s}
...
%\paragraph{\textls*{\small SSD}\hspace{0.5pt}s assume locality.} In our benchmarks, we find the average request size to be
\noindent{\bf \textls*{\small SSD}\hspace{0.5pt}s assume locality.} In our benchmarks, we find the average request size to be
less than 64 bytes. Today's \textls*{\small SSD}\hspace{0.5pt}s are optimized for reads in the
kilobyte range.  

%\paragraph{\textls*{\small SSD}\hspace{0.5pt}s have a low request rate.} The fastest~\cite{fusionio} can
\noindent{\bf \textls*{\small SSD}\hspace{0.5pt}s have a low request rate.} The fastest~\cite{fusionio} can
...
%\paragraph{\textls*{\small SSD}\hspace{0.5pt}s have high overhead.} Commodity \textls*{\small SSD}\hspace{0.5pt}s are usually treated
\noindent{\bf \textls*{\small SSD}\hspace{0.5pt}s have high overhead.} Commodity \textls*{\small SSD}\hspace{0.5pt}s are usually treated
...
A natural first step would be to add one or more \textls*{\small SSD}\hspace{0.5pt}s to each Grappa
...
reasons. First, the \textls*{\small SSD} must accept requests at a rate sufficient to
keep the cores busy. Grappa's target is for each cluster node to
generate 100 million random 8-byte requests per second, but most \textls*{\small SSD}\hspace{0.5pt}s
...
100 million per second, the \textls*{\small SSD}\hspace{0.5pt}s must support 2000 concurrent
requests. Again, existing \textls*{\small SSD}\hspace{0.5pt}s support orders of magnitude less request concurrency.

We see three ways to address this disparity.

%\paragraph{Scaling up the Flash array}
\noindent{\bf Scaling up the Flash array}
Existing Flash-based \textls*{\small SSD}\hspace{0.5pt}s are designed for large block requests. An
...
Much recent research has explored the use of \textls*{\small PCM} as a main memory
...
Grappa could also support a \textls*{\small PCM} array in a separate address space (for
...
amount have completed, the \textls*{\small PCM} array would write the results to \textls*{\small DRAM}\@,

...
Grappa context switches between tasks non-preemptively. \TODO{too
    vague: Most calls
    into the Grappa library, however, do initiate a context switch}, and
...
context switch time to be only \checkme{40ns}.  This time is kept
...
or Grappa determines it is profitable to do so.  \TODO{How does Grappa
    determine this? BrandonM: right now we don't actually spawn beyond
the starting number of stacks, so Grappa only bounds when there exist
idle worker and unbound task}.

\TODO{some of this first sentence might be in the part where we discuss our high
    level solutions to irregular dynamic parallelism}
...
Grappa employs work stealing: if a \checkme{grappa-process} determines it
does not have enough work, it takes some from a \checkme{grappa-process}

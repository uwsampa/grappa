...
Because it has to be live across the entire application.  All real applications do some form of floating point, however minor... -M
...
When a \checkme{grappa-process} runs out of work, it becomes a ``thief''
and asks another other \checkme{grappa-process} (``victim'') for work.
...
\comment{
where to put the following related? -BM
...
hmm..  -steal from FIFO side of queue
hmmm... -steal when at the moment readyQ empty and task queues empty
done -steal min(chunk\_size, half)

omit - Recent work has explored scalable work stealing in terms of efficient

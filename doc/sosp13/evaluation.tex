...
\end{tabular}
\end{center}
\caption{Comparing \Grappa with XMT, hand-tuned MPI and UPC. Numbers are presented as throughput normalized to \Grappa.}
...
In UTS, we obtained the best throughput by flushing frequently: on the order 100 microseconds , but
this mean
...
least 500 microseconds apart, but also flushing whenever the core went
idle. This leads to a large number of messages that are not 
...
The bimodal nature of the BFS distribution is due to a locality optimization that o

Both 


In BFS we exploit a large
 Since we are traversing power-law graphs,
some vertices have a very high number of edges. We exploit locality in
BFS by having tasks visit vertices whose edge lists are stored on the
local node. When this local traversal hits a high-degree vertex, BFS
is able to generate a large number of 

Figure \ref{fig:grappa-latency} shows

We see different aggregation behavior between BFS and UTS: 



In BFS, the bulk of remote accesses are done to take owner

Conversely, UTS attempts to 

...
\subsection{Fast User-Level Context Switching }
...
% More specifically, each context is cache lines: 1 for the worker struct and 3
% for stack data. So the are so 8 cacheline transfers per context switch (write
% the old context, read the next in). The bandwidth of single socket is 270M cache lines/s.
% 
% This asymptotically approaches
% 33.75Mcontexts/s as you increase the number of contexts (fewer and fewer in
% L3). Note that only 4+ cores reaches full rate because these westmere chips
% are balanced to not achieve full off-chip bandwidth until 4 cores.

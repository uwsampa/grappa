...
published results~\footnote{\url{http://icl.cs.utk.edu/hpcc/hpcc_results.cgi}}
for other high-end HPC systems.
...
\paragraph{Putting it all together with Unbalanced Tree Search (UTS).} We now
show the overall performance of \Grappa running UTS.
Figure~\ref{fig:grappa-uts}. The point of this experiment is to show that
\Grappa's context switching and communication layers can in fact be used
together to run an irregular application efficiently. Looking at the
performance as the number of nodes grows shows \TODO{...}.

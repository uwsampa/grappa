...
Visiting vertices in the distributed tree requires mostly remote accesses, and because each vertex in the tree must be visited before it can be expanded, blocking remote reads are required.
...
original benchmark. T1x trees are very shallow and wide, providing plentiful parallelism, while T3x
trees are very deep, causing the critical path to search T3x trees to be very long. On such
...
% . When the access to each vertex is a random access
\begin{figure}[ht]
    \begin{center}

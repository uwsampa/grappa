...
represented by a function pointer and its arguments. When a new task is
spawned, it is enqueued until it is ready for execution. Each hardware core
has a single operating system thread pinned to it. A {\em worker} is a
user-level execution thread (stack, \TODO{what else?}) to which tasks are
mapped onto when they are ready to execute. 



Once a task is mapped to worker it stays with that worker until it completes. 
During execution, a worker yields control of the processor core whenever it
performs a long-latency operation, allowing the processor to remain busy while
waiting for the operation to complete.
In addition, a programmer can direct
...
core of \Grappa's latency tolerance properties.


\TODO{We should expand on the cleverness of our scheduler?}

% talk about lifetime of a task. activating. blocking. work stealing. look at
% Mark's email. Holt's chat.

...
Grappa~\cite{grappa} prototype, a runtime system for irregular
...
comprehensive benchmark suite of irregular and graph applications. We also
...
latency, thus we will explore using solid state disks as main memory, since
and further take advantage of Grappa's latency tolerance properties.
...
Efficiently executing irregular computations on commodity hardware is \emph{hard}.  Much about commodity hardware is seemingly designed for the opposite application characteristics: caches assume applications have locality; networks are built with low injection rates but high bandwidth, assuming large packet sizes; even the runtimes and operating system is built assuming relatively infrequent communication and synchronization compared to computation.  This proposal rests on the seemingly outlandish claim that this commodity hardware / software stack can be convinced, through another layer of software, into efficiently executing irregular applications that lack locality, and communicate frequently with small amounts of data.  A folksy way of saying it is we think we can jam a square peg through a round hole.  Fortunately we have some preliminary evidence we won't get wedged along the way.  This is an intellectually challenging endeavor, and one square in the middle of the NSF XPS program: using and exploiting parallelism from irregular computations to efficiently execute them at scale.
...
%I think we should pitch what have been done so far as essays and a %bit part of the research effort is putting everything together. -Luis
...
\paragraph{Virtual topology:} Currently Grappa uses a flat network topology where each processing core in the system is individually addressed. As more nodes are added to the system, a quadratic increase in the number of threads (in aggregate across all nodes) is required in order to compensate for the \emph{same} network delay.  This is because the current aggregator treats each of these end-points individually, i.e., it only aggregates messages that go to the same exact end-points. This does not scale as the number of nodes go up because the probability of messages going to the same destination goes down (assuming a somewhat uniform communication distribution). In summary, this past design choice is simple but does not scale.
...
believe only \BigO{log(n)} more, hence a real \BigO{n / log(n)^2} scaling
benefit is achievable with the a virtual hierarchical network topology.

% Should we discuss doing HW support for aggregation? Using NetFPGAs?
...
This interface suffers from multiple shortcomings.  The biggest of which is that ultimately, moving data to and from a remote node constructively requires higher level semantic operations than simple reads and writes.  These higher level semantics can, of course, be synthesized from the underlying read and write primitives, but it wastes round trips and synchronization time.  One possible alternative solution is to borrow ideas from Active Messages~\cite{active-messages} and enable remote nodes to dispatch, essentially, user-level function calls.  The challenge here is to make those calls interoperate securely with a modern operating system, and a user-level runtime system. Additionally, the application runtime should be able to control how RDMA operations are carried out and make then semantically richer, e.g., do collective operations (min, max, average) on remote data, or hash-table gets and inserts.

% \comment{This seems weak.  Need some more ideas here.  -Mark. Luis: Isn't the problem that the app runtime should be able to determine how the RDMA ops should be done? Maybe add examples here like accessing a hash-table, doing collective operations on remote data etc (min, max, avg)? How about delegating simple operations to the network card? Maybe argue that getting the attention of the remote processor for these operations is too costly?} 
...
Luis: Ehm, it wasnt a rant, we are not even sure what our model is and this will take work... and before we know it, we cant cleanly do optimization. As you wish, leave it out, the current state is pretty sloppy... so if we get a comment on this, you will buy beers. 
...
Our research plan can be divided horizontally into three core areas:
programming languages/compilers; communication and storage; and benchmarking. Below we outline how we
plan to advance these fronts over the next four years.

\vspace{2ex}

%% TODO: need to expand below. 

\noindent\textbf{Year 1:} Benchmarks. Crude language support.

\vspace{1ex}
\noindent\textbf{Year 2:} Benchmarks. Hierarchical aggregation. Compiler optimizations.

\vspace{1ex}
\noindent\textbf{Year 3:} Hierarchical aggregation. SSD-only data structures.

\vspace{1ex}
\noindent\textbf{Year 4:} SSD-only data structures. Detailed system evaluation. 

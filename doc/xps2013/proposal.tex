...
performance of irregular applications on these mass market systems. Our goal
...
In this proposal we describe a body of research that extends an early
prototype of Grappa~\cite{grappa}, a runtime system for irregular
...
The work on Grappa is still in its infancy, however. This research proposal
seeks funding to extend Grappa in the following directions:
...
\item \textbf{Support for \ssd\ only memory systems:} The size of data being processed by large systems today (hundreds of terabytes) dwarfs what can fit into a single system. Multiple system nodes are required to maintain and access all of this data efficiently.  However, even with hundreds of channels to persistent storage, loading this data into DRAM can prove a bottleneck.  The Grappa runtime is designed to make applications tolerate extremely high memory latency, thus we will explore using solid state disks as main memory, since and further take advantage of Grappa's latency tolerance properties. 
...
Luis: Ehm, gup you! It wasnt a rant, we are not even sure what our model is and this will take work... and before we know it, we cant cleanly do optimization. As you wish, leave it out, the current state is pretty sloppy... so if we get a comment on this, you will buy beers. 

...
A \emph{proxy} object is instantiated ahead of time on each core and each worker merges its request into that structure before blocking or becoming the combiner.
Each global data structure must define a \emph{proxy} with the following:
\begin{enumerate}
...
The GlobalQueue has nearly the same implementation as the stack, but is unable to match locally, so \texttt{sync} first commits the pushes, then the pops.

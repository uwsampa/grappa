...
\vspace{1ex}
\textit{Explicit buffering.}
\TODO{de-emphasize buffering? requires rewording above sections to talk about Delegate ops as the primary form of use, and then we can mention this functionality offhand}
\Grappa provides an API to fetch a global pointer of
...
block until all responses have occurred. In \Grappa, buffered regions are
...
occurs before acquiring or releasing buffers.

Figure~\ref{fig:delegate-cache} depicts an example of how explicit
buffering and
...
\TODO{Redo this figure to simplify it and focus on delegates and latency tolerance.}
...
updates visible to all cores across the system in the same order. In addition, only one synchronous delegate will be in flight at a time from a particular task. Therefore, synchronization operations are not subject to potential reordering. 

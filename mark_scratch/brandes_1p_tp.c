...
double *compute_betweenness_centrality(
    ...
    i = 0; 
    task_scheduler(outer_tasks,
                   outer_task_data,
                   number_of_outer_tasks,
                   outer_task_data_t) {
        if (i >= cut_off)
            end_task();
        cd->i = i;
        ...
        ++i;
        cd->s_index = 0;
        cd->change_set_index = 0;
        
        cd->sd[cd->i].shortest_path_count = 1.0;
        cd->sd[cd->i].shortest_path_length = 0.0;
        cd->q_head = cd->q_tail = 0;
        
        // enqueue s to Q
        cd->q[cd->q_head] = cd->i;
        ++cd->q_head;
        while (cd->q_head != cd->q_tail) {
            // dequeue v from Q
            cd->v = cd->q[cd->q_tail];
            ++cd->q_tail;

...
/*  if (argc==6) {
  ...
  for (int th=0; th<num_threads; th++) {
	  if (th < thr_per_sock) {
		  ...
		  sp[th] = new SplitPhase(sock0_qs_toDel[th], sock0_qs_fromDel[th], green_per_ht, &glob_mem0);
	  } else {
		  ...
		  sp[th] = new SplitPhase(sock1_qs_toDel[ind], sock1_qs_fromDel[ind], green_per_ht, &glob_mem1);
	  }
  }

...
*   Source File:     generator/graph_generator.c
*   Program Library: xmt-csr/xmt-csr.pl
*   Module:          graph_generator.o
********************************************************************************
...
++ function mrg_get_uint_orig inlined
        |         if (/* Unlikely */ val < limit) {
        |                 do {
    1 X |                         val = mrg_get_uint_orig(st);
++ function mrg_get_uint_orig inlined
    1 X |                 } while (val < limit);
        |         }
...
** debug level: off
    ...
    2 X |         while (nverts > 1) {
    2 X |                 int square = generate_4way_bernoulli(st, level, lgN);
** function with unknown side effects: generate_4way_bernoulli
    2 X |                 int src_offset = square / 2;
    2 X |                 int tgt_offset = square % 2;
        |                 assert (base_src <= base_tgt);
    2 X |                 if (base_src == base_tgt) {
        |                         /* Clip-and-flip for undirected graph */
    2 X |                         if (src_offset > tgt_offset) {
        |                                 int temp = src_offset;
    ...
    2 X |                 nverts /= 2;
    2 X |                 ++level;
    2 X |                 base_src += nverts * src_offset;
    2 X |                 base_tgt += nverts * tgt_offset;
        |         }
        |         write_edge(result,
++ function write_edge inlined
++ function scramble inlined
++ function scramble inlined
...
++ function mrg_get_uint_orig inlined
    ...
    4 p |                 mrg_state new_state = state;
    4 D |                 mrg_skip(&new_state, 0, ei, 0);
    4 D |                 make_one_edge(nverts, 0, logN, &new_state, edges + (ei - start_edge), val0, val1);
        |         }
        | }
        | 
...
Loop   1 in generate_4way_bernoulli at line 47
       Loop summary: 0 loads, 0 stores, 0 floating point operations
		8 instructions, needs 30 streams for full utilization
		pipelined
       3 instructions added to satisfy dependences

Loop   2 in make_one_edge at line 149
       Loop not pipelined: not structurally ok
       Scheduled to minimize serial time
       Loop summary: 20 instructions, 0 floating point operations
		0 loads, 0 stores, 2 reloads, 0 spills, 2 branches, 1 calls

Parallel region   3 in generate_kronecker_range
       Multiple processor implementation
       Requesting at least 60 streams

Loop   4 in generate_kronecker_range at line 193 in region 3
       In parallel phase 1
       Block scheduled
       Loop not pipelined: not structurally ok
       Loop summary: 22 instructions, 0 floating point operations
		5 loads, 5 stores, 8 reloads, 1 spills, 1 branches, 2 calls

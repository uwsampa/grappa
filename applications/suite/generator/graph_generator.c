...
void generate_kronecker_range(
                ...
                GlobalAddress<graphint> edgeStarts, GlobalAddress<graphint> edgeEnds) {
  ...
  Grappa::forall_localized(edgeStarts+start_edge, end_edge-start_edge,
    [nverts, logN, state, val0, val1, edgeEnds](int64_t index, graphint& eStart) {
      ...
      packed_edge e;
      make_one_edge(nverts, 0, logN, &new_state, &e, val0, val1);
      
      eStart = e.v0;
      Grappa::delegate::write(edgeEnds+index, e.v1);
    }
  );  
}
#else

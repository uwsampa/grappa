...
double centrality(graph *g, double *bc, graphint Vs, int64_t* total_nedge) {
  ...
  double *delta  = (double *) xmalloc(NV * sizeof(double));
...
//    deprintn(" ]\n");
    ...
    for ( ; nQ > 1; nQ --) {
      ...
      MTA("mta block dynamic schedule")
      MTA("mta assert no alias *sigma *Q *bc *delta *start *QHead")
      for (j = Qstart; j < Qend; j++) {
        graphint v = Q[j]; 
        graphint myStart = start[v];
        graphint myEnd   = start[v+1];//myStart + child_count[v];
        double sum = 0;
        double sigma_v = (double) sigma[v];
        for (k = myStart; k < myEnd; k++) {
          graphint w = eV[k];//child[k];

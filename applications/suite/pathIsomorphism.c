...
static graphint checkEdgesRecursive(const graph* g, graphint v, const color_t* c) {
  const graphint * restrict edge = g->edgeStart; /* Edge domain */
  const graphint * restrict eV = g->endVertex; /* Edge domain */
...
//      fprintf(stderr, "%lld -> %lld (%lld)\n", v, eV[i], *c);
      if (c[1] == END) {
        ...
        result += checkEdgesRecursive(g, eV[i], c+1);
      }
    }
  }
  return result;
...
graphint pathIsomorphismPar(const graph* g, color_t* pattern) {
  ...
  MTA("mta assert parallel") OMP("omp parallel for reduction(+:nm)")
  for (graphint i=0; i<NV; i++) {
    if (marks[i] == *pattern) {
      nm += checkEdgesRecursive(g, i, pattern+1);
    }
  }

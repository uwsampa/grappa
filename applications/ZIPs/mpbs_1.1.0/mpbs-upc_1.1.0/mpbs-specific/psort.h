 ...
 *          PE's portion of the sorted data.  Resides on the shared heap and must be 
 *          freed by the calling function via upc_free.
 */ 
shared uint64_t * psortui64( int npes, int id, 
			     int bbits, uint64_t bssize, 
			     int *gmax, 
			     int num, 
			     uint64_t *data, 
			     shared uint64_t *recvs, char *pt_bsort );

...
*********1*****|***2*********3*********4*********5*********6*********7**/
{
   ...
   ********************************/
    ...
    */
#if defined(_USE_O_DIRECT) && defined(_UPC_)
   ...
   if( npes > nf )
   {
      if( id == 0 )
      {
         printf("Number of process elements exceeds number of files!\n");
         help();
      }
      else
         exit( 1 );
   ...
   if( npes > rb )
   {
      if( id == 0 )
      {
         printf( "Number of process elements exceeds records per" );
         printf( " bucket!\n" );
         help();
      }
      else
...
#if defined(_USE_O_DIRECT)
   if( rs == 4 ) {
     if (id == 0)
       printf("ERROR: Direct IO is only supported with 8 byte record size (-rs 8)\n");
     goto cleanup;
   }
#endif
...
int cmdline_specific(int na, char *args[] ) {
  /*
   * No command line arguments specific to UPC yet.
   */
  return 1;
}
...
void header_specific( uint32 np ) {
  /*
   * No specific header information yet.
   */
  return;
}
...
void trailer_specific( timer *t ) {
  /*
   * No specific trailer information yet.
   */
  return;
}

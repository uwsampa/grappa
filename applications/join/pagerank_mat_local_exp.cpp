...
// local aggregation is worst case O(|V|) storage on every core!
 // for Twitter this is 640MB/core * 16cores = 10GB
 // alternative is flat-combining
 on_all_cores([=] {
     ...
     for (auto it = vec.begin(); it!=vec.end(); ++it) {
        ...
        auto key = t_001.get(1);
        auto val = t_001.get(6);
        auto Init = 0;
        std::pair<int64_t,int64_t> entry(key, Init);
        auto res = local_group_hash_000.insert(entry); auto resIt = res.first; //auto resNew = res.second;
        // perform the update in place
        resIt->second = Aggregates::SUM<int64_t,int64_t>(resIt->second, val);
     }
     // note that I don't need a barrier here before all-to-all,
     // but for now I do for timing breakdown purposes
  });

...
#else
    a1 = (uint64 *) malloc(size*8 + 1024);
    a2 = (uint64 *) malloc(size*8 + 1024);
    a3 = (uint64 *) malloc(size*8 + 1024);
    ae_sum = (uint64 *) malloc(4*8);
#endif
...
if ((((uint64)a1 >> 6) & 0x7) != 0) printf(">>>> a1 not aligned\n");
if ((((uint64)a2 >> 6) & 0x7) != 0) printf(">>>> a2 not aligned\n");
if ((((uint64)a3 >> 6) & 0x7) != 0) printf(">>>> a3 not aligned\n");
...
//sleep(2);
  for (i = 0; i < size; i++) {
    ...
    exp_sum += a1[i] + a2[i];
    //printf("a1[%d]=%lld + a2[%d]=%lld = a3[%d]=%lld", i, a1[i], i, a2[i], i, a3[i]);
    if (a3[i] != (a1[i] + a2[i])) {
      printf("ERROR:  a1[%d]=%lld + a2[%d]=%lld != a3[%d]=%lld\n", i, a1[i], i, a2[i], i, a3[i]);
      error_count++;
    }
  }

  if ((act_sum != exp_sum) || (error_count > 0)) {
    printf("Sample 1 test FAILED - exp_sum=%lld act_sum=%lld error_count=%d\n", 
            exp_sum, act_sum, error_count);
  } else {

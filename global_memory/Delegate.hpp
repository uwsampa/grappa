...
//        printf("Delegate: iteration of handle_responses\n");
        ...
        if (r_resp) {
            printf("Delegate got RemoteResponse descriptor(%lx)\n", (uint64_t)r_resp);
            ...
            printf("Delegate produced descriptor(%lx)(data=%lx,full=%d) to core %u\n", (uint64_t)r_resp, r_resp->getData(), r_resp->full, r_resp->getCoreId());
        }
        thread_yield(me);
    }
}
...
//        printf("Delegate: iteration of handle_remote_requests\n");
        ...
        if (r_msg_valid) {
            ...
            printf("Delegate got RemoteRequest from %u; addr=%lx,op=%d,data=%lu,descriptor(%lx)\n", r_fromid, (uint64_t)addr,operation,data,(uint64_t)r_req);
            ...
            switch(operation) {
                case READ: {
                   ...
                   printf("Delegate sent response to %u; data=%lx\n", r_fromid, resp.getData());
                   break;
                }
                default:
                    assert(false);
...
//        printf("Delegate: iteration of handle_local_requests\n");
        ...
        while (iq->tryConsume(&data)) {
            printf("Delegate got local request\n");
            ...
            switch (md->getOperation()) {
                case READ: {
                    if (isLocal) {
                        ...
                        printf("Delegate got read to local; addr=%lx\n", (uint64_t)addr);
                        // TODO pref/yield?
